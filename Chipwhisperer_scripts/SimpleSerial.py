...
class SimpleSerial(TargetTemplate, util.DisableNewAttr):
    ...
    def readOutput(self):
        ...
        #####
        #   SPI FLASH SIM ciphertext injection for dpa
        #####
        ...
        spisim_ser = serial.Serial(spisim_port, spisim_baud, timeout=1)

        # open the serial port
        if spisim_ser.isOpen():
            logging.info(spisim_ser.name + ' is open...')
            spisim_ser.write(':') # start byte sig
            spisim_ser.write(self.input) # cipher text data 0x10 in size
            spisim_ser.write('!') # end byte sig after 0x10 of data recvd
            spisim_ser.close()
